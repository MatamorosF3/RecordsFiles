...
int TDAIndexFile::readrecord( char* rec, int offset){
...
//OJO
int TDAIndexFile::writerecord(const char *buffer, int ind){
...
int TDAIndexFile::findrecord(int id){
    /*TDAFile f;
...
int TDAIndexFile::findrecord(const char* rec,int ind){

    return 0;
}

void TDAIndexFile::seek(int ind){
    TDAFile::seek(ind);

}

void TDAIndexFile::seek(ios::seekdir mode)
{
    TDAFile::seek(mode);
}

int TDAIndexFile::tell(){

    return TDAFile::tell();

}

int TDAIndexFile::eraserecord(int ind){
    //avail.push_back(ind);
    this->open("Clientes.txt",ios_base::in | ios_base::out);
    if(ind == 1){
        this->seek(0);
...
int TDAIndexFile::updaterecord(const char* rec, int ind){
    this->open("Clientes.txt",ios_base::in | ios_base::out);
    if(ind == 1){
        this->seek(0);
    }
